...
func (t *AssertionTestCaseProvider) GPUPostCallTest(workers []*FuzzerWorker, gpuResult *coverage.GPUExecutionResult, markerOffsets []int32, bigIntWeightValue *big.Int) (bool, error) {
	...
	// bugPCsProcessed := make(map[uint32]bool)
	// newShrinkRequests := make([][]ShrinkCallSequenceRequest, len(workers))
	...
	// 	newShrinkRequests[workerIndex] = make([]ShrinkCallSequenceRequest, 0)

	// 	// Update metrics even for empty sequence if original logic implies processing
	// 	worker.workerMetrics().callsTested.Add(worker.workerMetrics().callsTested, big.NewInt(int64(total_calls_tested_per_worker)))
